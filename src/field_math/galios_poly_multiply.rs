    ...
    loop {
        match polys_iter.next() {
            Some(poly_multiplier) => {
                    ...
                    for multiplier_pitem in &multiplier_pitems {
                        result_pitems.push(
                            Pitem {
                                ...
                                coe: galios_num_multiply(multiplier_pitem.coe, multiplicand_pitem.coe, &gs),
                            }
                        );
                    }
                ...
                poly_multiplicand = galios_poly_add(result_polys.iter().map(|poly| poly.as_str()).collect());
